...
import { MainStackScreenNames, RootStackParams } from '../router';

export const navigationRef =
  createRef<NavigationContainerRef<RootStackParams>>();

export const navigation = {
  replace: (to: string, params?: { [key: string]: string | number }) => {
    navigationRef.current?.dispatch(StackActions.replace(to, params));
  },
  push: (to: MainStackScreenNames, params?: { [key: string]: unknown }) => {
